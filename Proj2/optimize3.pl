...
getDist(Vans, D):-
	...
	getFarmacies(N),
    maximum(B, Vans),
    getLists(FullList, [], Vans, 0, B),
    calc_dist(N, FullList, D).
...
getLists(FL, FL, _, N, N).
getLists(FullList, TempFullList, Vans, Now, B):-
    getN(L, [], Vans, 0, Now),
    append(TempFullList, [L], New_l),
    Now1 is Now + 1,
    getLists(FullList, New_l, Vans, Now1, B).
    

getN(List, Templ, V, 0, Now):-
    append(Templ, [0], TempL),
    getN(List, TempL, V, 1, Now).
getN(List, List, [], _, _).
getN(List, Templ, [B|T], Ind, Now):-
    (B=Now->
    append(Templ, [Ind], TempL),
    Ind1 is Ind + 1,
    getN(List, TempL, T, Ind1, Now);
    Ind1 is Ind + 1,
    getN(List, Templ, T, Ind1, Now)).
